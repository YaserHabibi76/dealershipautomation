...
const CSV_OUT = process.env.SCRAPE_OUTPUT || path.resolve(__dirname, 'toyota_ontario_dealerships.csv');
const HEADED  = process.argv.includes('--headed');

// Toyota Canada dealersList API — triggered when user types a province name
const DEALERS_API = 'https://www.toyota.ca/bin/find_a_dealer/dealersList'
  + '?brand=toyota&language=en&userInput=Ontario'
...
async function fetchDealers() {
  const res = await fetch(DEALERS_API, {
    headers: {
      ...
      'Referer': 'https://www.toyota.ca/en/dealers/',
    },
  });
...
(async () => {
  console.log('Fetching Ontario Toyota dealers from Toyota Canada API…');
